...
<head>
    ...
    <meta name="description" content="@yield('meta_description', 'Ali Enes Eren - Full Stack Developer | PHP, Laravel, Vue.js Uzmanı')">
    <meta property="og:title" content="@yield('title', 'Alienes.me - Portfolio')">
    <meta property="og:description" content="@yield('meta_description', 'Ali Enes Eren - Full Stack Developer | PHP, Laravel, Vue.js Uzmanı')">
    ...
    @php
        $profile = \Illuminate\Support\Facades\Cache::remember('profile_first', now()->addHours(24), fn() => \App\Models\Profile::first());
        $faviconUrl = $profile && $profile->github_avatar_url 
            ? $profile->github_avatar_url 
            : asset('favicon.ico');
        
        // Check if sections have content
        $hasExperiences = \Illuminate\Support\Facades\Cache::remember('has_experiences', now()->addHours(24), fn() => \App\Models\Experience::count() > 0);
        $hasSkills = \Illuminate\Support\Facades\Cache::remember('has_skills', now()->addHours(24), fn() => \App\Models\Skill::count() > 0);
        $hasProjects = \Illuminate\Support\Facades\Cache::remember('has_projects', now()->addHours(24), fn() => \App\Models\Project::count() > 0);
        $hasCv = \Illuminate\Support\Facades\Cache::remember('has_published_cv', now()->addHours(24), fn() => \App\Models\Cv::where('is_published', true)->exists());
    @endphp
    ...
    <nav id="navbar" class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-white dark:bg-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <a href="{{ route('study.index') }}" class="text-gray-700 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400 transition">📚 Çalışma Notları</a>
        ...
        <div id="mobileMenu" class="hidden md:hidden bg-white dark:bg-gray-800 border-t dark:border-gray-700">
            <div class="px-4 py-4 space-y-3">
                ...
                <a href="{{ route('study.index') }}" class="block text-gray-700 dark:text-gray-300 hover:text-primary-600">📚 Çalışma Notları</a>
