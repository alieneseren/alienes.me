...
<div class="container mx-auto px-4 py-8 sm:py-12">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <a href="{{ route('study.category', $category->slug) }}" 
               class="block bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl transition-shadow p-5 sm:p-6 border border-gray-200 dark:border-gray-700">
                <div class="text-4xl sm:text-5xl mb-3 sm:mb-4">{{ $category->icon }}</div>
                <h2 class="text-xl sm:text-2xl font-bold mb-2 text-gray-900 dark:text-white break-words line-clamp-2">{{ $category->name }}</h2>
                <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-3 sm:mb-4 break-words line-clamp-2">{{ $category->description }}</p>
                <div class="flex items-center text-xs sm:text-sm text-gray-500 dark:text-gray-500">
                    ...
                    <span class="truncate">{{ $category->active_notes_count }} not</span>
                </div>
            </a>
            @endforeach
        </div>
