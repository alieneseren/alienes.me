...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Yeni Not Ekle</h1>
</div>

<div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
    <form action="{{ route('admin.study-notes.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ old('title') }}" required class="input-field" placeholder="Modül 1: Giriş">
            @error('title')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Açıklama</label>
            <textarea name="description" rows="3" class="input-field" placeholder="Kısa açıklama (opsiyonel)">{{ old('description') }}</textarea>
            @error('description')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Dosya *</label>
            <input type="file" name="file" required class="input-field" accept=".html,.htm,.pdf,.md">
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">HTML, PDF veya Markdown dosyası yükleyebilirsiniz (Max: 10MB)</p>
